...
import { countries, countrySlug, getCountry, placesInCountry, visitCount } from "../../../places";
...
export default async function CountryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const pieces = countryPlaces.reduce((sum, place) => sum + contentCountForPlace(place.slug), 0);
  ...
  return <main><Header active="places" />
    ...
    <section className="country-detail-index shell"><div className="country-detail-heading"><p className="eyebrow">The location index</p><h2>Choose a place</h2></div><div className="country-place-list">{countryPlaces.map((place, index) => <AtlasLink place={place.slug} href={`/places/${place.slug}`} key={place.slug}><span>{String(index + 1).padStart(2, "0")}</span><h3>{place.city}</h3><p>{contentCountForPlace(place.slug) > 0 ? `${contentCountForPlace(place.slug)} collected` : "File open"}</p><i>↗</i></AtlasLink>)}</div></section>
    <Footer />
  </main>;
}
